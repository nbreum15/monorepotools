...
public class FolderLevelPanel extends JPanel {

    private final JBTable table;

    public FolderLevelPanel(@NotNull final String title) {
        super(new BorderLayout());
        setBorder(GuiUtil.createTitledBorder(title));
        var model = new FolderLevelModel(new ArrayList<>());
    ...
    public List<FolderLevelDTO> getRows() {
        return getModel().getRows();
    }

    public void setRows(List<FolderLevelDTO> rows) {
        getRows().clear();
        getRows().addAll(rows);
    ...
    @NotNull
    private FolderLevelModel getModel() {
        return (FolderLevelModel) table.getModel();
    }

    private void doAdd() {
        getModel().getRows().add(FolderLevelDTO.builder().build());
        getModel().fireTableDataChanged();
    }
    ...
    private void doRemove() {
        ...
        if (index != null && index.length > 0) {
            final Set<FolderLevelDTO> toRemove = new HashSet<>();
